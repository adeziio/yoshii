...
def google_searcher(search):
    ...
    try:
        search_image = requests.get(
            "https://google-search3.p.rapidapi.com/api/v1/image/q="+search, headers=headers).json()['image_results'][0]
        image_url = search_image['image']['src']
    except:
        image_url = ""

    try:
        search = requests.get("https://google-search3.p.rapidapi.com/api/v1/search/q=" +
                              search, headers=headers).json()['results'][0]
